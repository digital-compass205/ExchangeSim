"""Translating one field's value between its string form and its wire form.

A field's meaning is one thing and its spelling is another, and every binary
protocol here disagrees with the string form somewhere: OCG-C spells
``OrderCapacity`` ``A``/``P`` in FIX and 1/2 in binary; NSE holds a price as an
integer number of paise and a time-in-force as a bit. A converter per field
keeps those differences in one table instead of scattered through a codec.
...
session for what every specification treats as an ordinary validation error.
"""

def scale(text, places):
    """A decimal string to a scaled integer, without ever touching a float.

    ``10.05`` at eight implied places is 1005000000, and at two it is 1005.
    ``places`` is required rather than defaulted, because every protocol here
    implies a different number of them. Parsing digits rather than
