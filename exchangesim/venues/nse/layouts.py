...
def build_cm():
    ...
    layouts.define(X.GR_RESPONSE, "MS_GR_RESPONSE", 136, (
        ...
        L.Field(D.CRYPTOGRAPHIC_KEY, "CryptographicKey", T.Char(32), 76,
                redact=True),
        L.Field(D.STATIC_IV, "StaticCryptographicIV", T.Char(8), 108,
                redact=True),
        L.Field(D.DYNAMIC_IV, "DynamicCryptographicIV", T.LONG_LONG, 116,
                redact=True),
        L.Field(D.ADDITIONAL_KEY, "CryptographicAdditionalKey", T.Char(12),
                124, redact=True),
    ))
