...
ASSUMPTIONS = [
    ...
    {"topic": "how a stock code is written",
     "behaviour": "a numeric SecurityID is matched with padding ignored, so "
                  "1, 0001 and 00001 all reach CK Hutchison; reference data, "
                  "reports, market data and the audit all say 1 -- the code "
                  "as a number, never zero-padded",
     "alternative": "carry the padded five-digit form HKEX's listing tables "
                    "print, or echo back whichever spelling the client sent",
     "source": "an SEHK code is a number and clients send it either way -- a "
               "real gateway sends 1. Which form the exchange *answers* with "
               "is not stated in either protocol document; one form is chosen "
