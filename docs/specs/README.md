...
| **A TLS 1.3 Gateway Router** on its own port issuing the key, IV and AAD (`GR_REQUEST 2400` / `GR_RESPONSE 2401`), authenticated by a self-signed CA the exchange distributes as `gr_ca_cert1.pem` (p.156) — server-side only, no client certificates | a second listener in `venues/nse/venue.py`, over `core/reactor.py`'s transport seam; `tls/` terminates TLS on a `MemoryBIO` and issues the CA and leaf itself, since the stdlib has `ssl` but nothing that writes a certificate |
