    ...
    def test_every_reply_to_a_signed_on_user_names_them(self):
        ...
        self.assertEqual(
            {X.SIGN_ON_REQUEST_OUT, X.SYSTEM_INFORMATION_OUT,
             ...
             X.ORDER_CANCEL_CONFIRMATION, X.HEADER_RECORD,
             X.MESSAGE_RECORD, X.TRAILER_RECORD,
...
class SystemInformationTest(unittest.TestCase):
    ...
    A real Futures & Options client sets its streams up from it once and
    asserts on a second, so every 1601 it did not ask for is a crash. There
    were three: one on sign-on, one on every market state change, and the
    download replaying both. This venue shares the client code path.
    """
...
class MessageDownloadTest(unittest.TestCase):
    ...
    def test_what_a_user_missed_while_signed_off_is_recoverable(self):
        ...
        self.client.new_order(USER_TWO, side=D.BuySell.SELL, quantity=100, price="1500.00")
        ...
        aggressor.new_order(USER_THREE, side=D.BuySell.BUY, quantity=100, price="1500.00")
